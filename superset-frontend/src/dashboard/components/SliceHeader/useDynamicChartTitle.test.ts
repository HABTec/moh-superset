...
test('appends Indicator, Dataset and Year selections in order', () => {
  const filters = {
...
test('prefers an in-scope filter when the first name match is out of scope', () => {
  ...
  const filters = {
    DS_A: buildFilter('DS_A', 'Dataset', undefined, tabAScope),
    DS_B: buildFilter('DS_B', 'Dataset', undefined, tabBScope),
  };
  const dataMask = {
...
test('includes a filter default value in the title', () => {
  const filters = {
...
test('includes defaults alongside actively changed filters', () => {
  const filters = {
    IND: buildFilter('IND', 'Indicator', ['Plasmodium']),
    YR: buildFilter('YR', 'Year', ['2024']),
    QTR: buildFilter('QTR', 'Quarter'),
